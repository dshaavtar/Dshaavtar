...
export const ORDER_STATUS_MAP: Record<string, BadgeConfig> = {
...
const VERIFICATION_STATUS_MAP: Record<string, BadgeConfig> = {
  verified: { label: "Verified", variant: "success" },
  pending: { label: "Pending", variant: "warning" },
  rejected: { label: "Rejected", variant: "error" },
};

const NOTIFICATION_STATUS_MAP: Record<string, BadgeConfig> = {
...
interface StatusBadgeProps {
  type: "order" | "verification" | "notification" | "boolean" | "custom";
...
export default function StatusBadge({
  ...
  if (type === "boolean") {
    config = value
      ? { label: trueLabel, variant: "success" }
      : { label: falseLabel, variant: "neutral" };
  } else if (type === "order") {
    config = ORDER_STATUS_MAP[value as string] ?? {
      label: String(value),
      variant: "neutral",
    };
  } else if (type === "verification") {
    config = VERIFICATION_STATUS_MAP[value as string] ?? {
      label: String(value),
      variant: "neutral",
    };
  } else if (type === "notification") {
    config = NOTIFICATION_STATUS_MAP[value as string] ?? {
      label: String(value),
      variant: "neutral",
    };
  } else {
